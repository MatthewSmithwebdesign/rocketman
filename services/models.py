...
class ServicePage(Page):
    ...
    service_image = models.ForeignKey(
        ...
        blank=False,
        on_delete=models.SET_NULL,
        help_text="this image will be used on the service listings page ",
        related_name="+",
    )
    ...
    def clean(self):
            ...
            raise ValidationError(
                {
                    "internal_page": ValidationError(
                        "Please only select a page Or an external URL"
                    ),
        ...
        if not self.internal_page and not self.external_page:
            raise ValidationError(
                {
                    "key": ValidationError(" You must always select a page Or add a external URL"),
                    "key2": ValidationError( " You must always select a page Or add a external URL"),
                }
            )
